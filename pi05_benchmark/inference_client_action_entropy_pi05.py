...
def main():
    ...
    # experiment
    parser.add_argument("--out_path", type=str, default="/home/sangfor/Videos/libero_output_pi0/action_entropy_test_debug", help="saved video results")
    ...
    policy = websocket_client_policy.WebsocketClientPolicy(host=args.host, port=PORT_MAP[args.port])
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while end_episode==False and trial_success==False:
                ...
                obs_dict = {}
                obs_dict['observation/wrist_image'] = obs["robot0_eye_in_hand_image"][::-1, ::-1, :]
                obs_dict['observation/image'] = obs["agentview_image"][::-1, ::-1, :]
                ...
                obs_dict["prompt"] = task_description
                
                # ------------------------setp 2 get action ----------------------------------------------- #
                pred_dict = policy.infer(obs_dict)
                pred_action_chunk = pred_dict['actions']
                # if len(pred_action_chunk.shape) == 2:
                #     pred_action_chunk = pred_action_chunk[None, ...]
                #     pred_action_dict["normalized_action"] = pred_dict["normalized_action"][None, ...]
                assert len(pred_action_chunk.shape) == 3, "action dimension is not 3"
                pred_action_dict= {}
                pred_action_dict["normalized_action"] = pred_dict["normalized_action"]
                pred_action_dict["action.end_effector_position"] = pred_action_chunk[:, :, :3]
                pred_action_dict["action.end_effector_rotation"] = pred_action_chunk[:, :, 3:6]
                pred_action_dict["action.gripper_close"] = pred_action_chunk[:, :, 6]
                ...
                for a_t in range(chunk_to_execute):
                    ...
                    action_dict["right_gripper"] = [pred_action_dict["action.gripper_close"][a_t]] # follow pi0.5, use continous values here, range [-1, 1]
